using AsciiArtDeployer;
using LibGit2Sharp;
using System;
using System.IO;
using System.Threading.Tasks;

namespace GitCommitTool
{

    class AsciiArtDeployer
    {
        const string PiDecimals = "";
        static async Task Main(string[] args)
        {
            var asciiArtFileName = args[0];
            var repositoryUrl = args[1];
            if (!File.Exists(asciiArtFileName))
            {
                Console.WriteLine("The first argument is not pointing to a text file.");
                return;
            }
            if (!Uri.TryCreate(args[1], UriKind.Absolute, out var _))
            {
                Console.WriteLine("The second argument is not an URL of a repository.");
                return;
            }
            var fullAsciiArt = await File.ReadAllTextAsync(asciiArtFileName);
            var asciiArtMatrix = fullAsciiArt.Split(Environment.NewLine);
            using var directory = new WorkspaceDirectory();

            var repositoryPath = Repository.Clone(repositoryUrl, directory.Path);
            using var repo = new Repository(repositoryPath);
            var fileName = directory.GetFilePath("Pi Ascii Art Deployer");

            File.WriteAllText(fileName, "3,");
            var remainingPiDecimals = PiDecimals;
            for (var indexWeeks = 0; indexWeeks < 52; indexWeeks++)
            {
                for (var indexDays = 0; indexDays < 7; indexDays++)
                {
                    var currentChar = asciiArtMatrix[indexDays][indexWeeks];
                    if (currentChar == ' ')
                    {
                        continue;
                    }
                    var diffDays = indexDays - 7;
                    var diffWeeks = indexWeeks - 51;
                    var totalDiffDays = diffDays + (7 * diffWeeks);
                    Commit(currentChar == 'X' ? 9 : 3, repo, fileName, DateTimeOffset.UtcNow.AddDays(totalDiffDays), ref remainingPiDecimals);
                }
            }
            repo.Network.Push(repo.Branches["main"]);
            Console.WriteLine("Deployed successfully");
        }

        static void Commit(int numberOfTimes, Repository repository, string fileName, DateTimeOffset time, ref string remainingPiDecimals)
        {
            for (var commitIndex = 0; commitIndex < numberOfTimes; commitIndex++)
            {
                var currentPiDecimal = remainingPiDecimals[0].ToString();
                remainingPiDecimals = remainingPiDecimals[1..];
                Commit(repository, fileName, currentPiDecimal, time);
            }
        }

        static void Commit(Repository repository, string fileName, string content, DateTimeOffset time)
        {
            File.AppendAllText(fileName, content);
            Commands.Stage(repository, fileName);
            repository.Commit("Another commit with a PI decimal", new Signature("Pi", "piday@shane.ro", time), new Signature("Pi", "piday@shane.ro", time));
        }
    }
}
